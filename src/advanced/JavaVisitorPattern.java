...
public class JavaVisitorPattern {
    ...
    private static Map<Integer, Set<Integer>> edgesMap = new HashMap<>();
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < n - 1; i++) {
            int u = sc.nextInt() - 1;
            int v = sc.nextInt() - 1;
            edgesMap.computeIfAbsent(u, k -> new HashSet<>()).add(v);
            edgesMap.computeIfAbsent(v, k -> new HashSet<>()).add(u);
        }
        sc.close();
    ...
    private static Tree buildTree(int nodeIndex, int depth) {
        Set<Integer> childrenIndices = edgesMap.getOrDefault(nodeIndex, new HashSet<>());
        if (childrenIndices.isEmpty()) {
            ...
            for (int childIndex : new ArrayList<>(childrenIndices)) {
                edgesMap.get(childIndex).remove(nodeIndex);
                node.addChild(buildTree(childIndex, depth + 1));
            }
            return node;
        }
    }
